...
@Composable fun SettingsScreen(activity:Activity,cats:List<Category>,onBack:()->Unit,onArchive:()->Unit,onMessage:(String)->Unit) {
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        LazyColumn(contentPadding=PaddingValues(18.dp),verticalArrangement=Arrangement.spacedBy(18.dp)) {
            ...
            item { Panel { Text("Hatırlatma izinleri",style=MaterialTheme.typography.titleMedium);ActionRow("Bildirim ayarları",Icons.Outlined.NotificationsNone){runCatching { activity.startActivity(Intent(Settings.ACTION_APP_NOTIFICATION_SETTINGS).putExtra(Settings.EXTRA_APP_PACKAGE,activity.packageName)) }};if(Build.VERSION.SDK_INT>=31)ActionRow("Alarm ve hatırlatma izni",Icons.Outlined.Alarm){runCatching { activity.startActivity(Intent(Settings.ACTION_REQUEST_SCHEDULE_EXACT_ALARM,Uri.parse("package:${activity.packageName}"))) }} } }
        }
    }
