...
class EditorState(initial:Note,var key:ByteArray?=null,initialContent:Content=initial.content()) {
    ...
    suspend fun persist() = mutex.withLock {
        ...
        try {
            if(saved && withContext(Dispatchers.IO){Repo.dao.get(note.id)!=null})return@withLock
...
class MainActivity:ComponentActivity() {
    ...
    override fun onCreate(state:Bundle?) { super.onCreate(state);Repo.init(this);enableEdgeToEdge();model=ViewModelProvider(this)[MainModel::class.java];if(state==null)model.incoming=intent.getStringExtra("note");setContent { AppRoot(this,model) } }
    override fun onNewIntent(i:Intent) { super.onNewIntent(i);model.incoming=i.getStringExtra("note") }
...
@Composable fun AppRoot(activity:MainActivity,model:MainModel) {
    ...
    val accent=remember(prefsVersion){Repo.str("accent","#c7c7db")};val animated=remember(prefsVersion){Repo.bool("animations",true)}
    ...
    NoteTheme(accent) {
        Scaffold(snackbarHost={SnackbarHost(snackbar)},containerColor=Bg) { insets ->
            Box(Modifier.fillMaxSize().padding(insets).imePadding()) {
                AnimatedContent(targetState=if(e!=null)"editor" else model.screen,transitionSpec={ if(animated)(fadeIn(spring())+slideInHorizontally { it/12 }) togetherWith fadeOut() else EnterTransition.None togetherWith ExitTransition.None },label="ekran") { screen ->
                    when(screen) {
                        ...
                        else -> HomeScreen(notes.filter { !it.archived },cats,prefsVersion,onOpen={open(it)},onCopy={open(it,true)},onHold={action=it},onSettings={model.screen="settings"},onNew={model.editor=EditorState(Note(category=cats.find { it.id=="personal" }?.id?:cats.firstOrNull()?.id?:"other",position=(notes.maxOfOrNull { it.position }?:0)+1))})
                    }
                }
            }
        }
...
@Composable fun HomeScreen(notes:List<Note>,cats:List<Category>,prefsVersion:Int,onOpen:(Note)->Unit,onCopy:(Note)->Unit,onHold:(Note)->Unit,onSettings:()->Unit,onNew:()->Unit) {
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        LazyRow(contentPadding=PaddingValues(horizontal=18.dp,vertical=10.dp),horizontalArrangement=Arrangement.spacedBy(8.dp)) { item { FilterChip(selected.isEmpty(),{selected=""},label={Text("Tümü")}) };items(ordered,key={it.id}) { c -> FilterChip(selected==c.id,{selected=c.id},label={Text(c.name)},leadingIcon={Box(Modifier.size(7.dp).background(hexColor(c.color),CircleShape))}) } }
...
@Composable fun NoteCard(n:Note,cat:Category?,onOpen:()->Unit,onCopy:()->Unit,onHold:()->Unit,modifier:Modifier=Modifier,trailing:@Composable ()->Unit={}) {
    ...
    Surface(modifier.fillMaxWidth().combinedClickable(onClick=onOpen,onLongClick=onHold),color=SurfaceColor,shape=RoundedCornerShape(22.dp),border=BorderStroke(1.dp,Line)) { Column(Modifier.padding(16.dp)) {
        ...
        Spacer(Modifier.height(9.dp));Row(verticalAlignment=Alignment.CenterVertically) {
            if(cat!=null)Text(cat.name,color=hexColor(cat.color),style=MaterialTheme.typography.labelSmall,modifier=Modifier.background(hexColor(cat.color).copy(alpha=0.1f),RoundedCornerShape(6.dp)).padding(horizontal=8.dp,vertical=4.dp))
            Spacer(Modifier.width(8.dp));Text(relativeTime(n.updated,now),style=MaterialTheme.typography.labelSmall,color=Muted,modifier=Modifier.weight(1f));if(n.reminder.isNotBlank())Icon(Icons.Outlined.NotificationsNone,"Hatırlatma",Modifier.size(16.dp),tint=Muted)
            IconButton(onClick=onCopy,modifier=Modifier.size(44.dp)){Icon(Icons.Outlined.ContentCopy,"Notu kopyala",Modifier.size(19.dp))};trailing()
        }
    } }
}
...
@Composable fun UnlockDialog(activity:Activity,n:Note,onDismiss:()->Unit,onUnlocked:(Content,ByteArray)->Unit) {
    var pin by remember { mutableStateOf("") };var error by remember { mutableStateOf("") };var busy by remember { mutableStateOf(false) };val scope=rememberCoroutineScope();var signal by remember { mutableStateOf<android.os.CancellationSignal?>(null) }
    DisposableEffect(Unit){onDispose { signal?.cancel() }}
    fun biometric() { try { val cipher=Vault.bioCipher(n.id,n.bio);signal=Vault.biometric(activity,cipher,{ authenticated -> runCatching { val d=Vault.bytes(n.bio);val key=authenticated.doFinal(d.copyOfRange(12,d.size));Content.parse(String(Vault.decrypt(n.payload,key))) to key }.onSuccess { onUnlocked(it.first,it.second) }.onFailure { error="Biyometrik anahtar kullanılamıyor. PIN ile açın." } },{error=it}) } catch(_:Exception){error="Biyometri kullanılamıyor. Kurtarma PIN'ini kullanın."} }
    AlertDialog(onDismissRequest={if(!busy)onDismiss()},title={Text("Notun kilidini aç")},text={Column(verticalArrangement=Arrangement.spacedBy(12.dp)) { Text("Bu notun PIN'ini girin.",color=Muted);OutlinedTextField(pin,{if(it.length<=12&&it.all(Char::isDigit))pin=it},label={Text("PIN")},singleLine=true,visualTransformation=PasswordVisualTransformation(),keyboardOptions=androidx.compose.foundation.text.KeyboardOptions(keyboardType=KeyboardType.NumberPassword));if(n.bio.isNotBlank()&&Vault.bioAvailable(activity))OutlinedButton(onClick=::biometric,modifier=Modifier.fillMaxWidth()){Icon(Icons.Outlined.Fingerprint,null);Spacer(Modifier.width(8.dp));Text("Biyometri ile aç")};if(error.isNotBlank())Text(error,color=MaterialTheme.colorScheme.error);if(busy)LinearProgressIndicator(Modifier.fillMaxWidth())}},confirmButton={TextButton(enabled=pin.length>=6&&!busy,onClick={busy=true;scope.launch {
        val wait=Repo.prefs.getLong("lockWait.${n.id}",0)-System.currentTimeMillis()
        if(wait>0){error="${(wait/1000)+1} saniye sonra tekrar deneyin.";busy=false;return@launch}
