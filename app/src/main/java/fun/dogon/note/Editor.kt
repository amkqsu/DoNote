...
@Composable fun EditorScreen(e:EditorState,cats:List<Category>,activity:Activity,onClose:()->Unit,onMessage:(String)->Unit,onCopy:()->Unit) {
    ...
    Column(Modifier.fillMaxSize()) {
        TopAppBar(title={Column { Text("Not",style=MaterialTheme.typography.titleMedium);Text(if(busy)"Resim hazırlanıyor…" else if(e.saved)"Kaydedildi" else "Kaydediliyor…",style=MaterialTheme.typography.labelSmall,color=Muted) }},navigationIcon={IconButton(onClick=onClose){Icon(Icons.Outlined.ArrowBack,"Kaydet ve geri dön")}},actions={IconButton(onClick=onCopy){Icon(Icons.Outlined.ContentCopy,"Notu kopyala")};IconButton(onClick={options=true}){Icon(Icons.Outlined.MoreVert,"Not seçenekleri")};DropdownMenu(options,{options=false}) { DropdownMenuItem(text={Text("İkon ve renk")},onClick={options=false;iconDialog=true});DropdownMenuItem(text={Text("Hatırlatma")},onClick={options=false;reminderDialog=true});DropdownMenuItem(text={Text(if(e.note.mode=="none")"Notu kilitle" else "Kilit ayarları")},onClick={options=false;lockDialog=true});DropdownMenuItem(text={Text(if(e.note.pinned)"Sabitlemeyi kaldır" else "Sabitle")},onClick={options=false;e.meta(e.note.copy(pinned=!e.note.pinned))});DropdownMenuItem(text={Text("Not içinde ara")},onClick={options=false;searchDialog=true});DropdownMenuItem(text={Text("Panodan yapıştır")},onClick={options=false;val clip=(activity.getSystemService(Context.CLIPBOARD_SERVICE) as android.content.ClipboardManager).primaryClip?.getItemAt(0)?.coerceToText(activity)?.toString().orEmpty();if(clip.isBlank())onMessage("Pano boş") else { val v=editView;if(v!=null&&v.hasFocus()){val st=v.selectionStart.coerceAtLeast(0);val en=v.selectionEnd.coerceAtLeast(st);v.text.replace(st,en,clip)}else{val b=Block(html=TextUtils.htmlEncode(clip).replace("\n","<br>"));e.change(e.content.copy(blocks=e.content.blocks+b));e.active=b.id}}});DropdownMenuItem(text={Text("Bağlantıları aç")},onClick={options=false;linksDialog=true}) } })
        Row(Modifier.padding(horizontal=18.dp),verticalAlignment=Alignment.CenterVertically) {
            ...
            Box { AssistChip(onClick={categoryMenu=true},label={Text(cats.find { it.id==e.note.category }?.name?:"Kategori")},trailingIcon={Icon(Icons.Outlined.ExpandMore,null,Modifier.size(16.dp))});DropdownMenu(categoryMenu,{categoryMenu=false}){cats.forEach { c -> DropdownMenuItem(text={Text(c.name)},leadingIcon={Icon(noteIcon(c.icon),null,Modifier.size(18.dp),tint=hexColor(c.color))},onClick={e.meta(e.note.copy(category=c.id));categoryMenu=false}) }} }
            Spacer(Modifier.weight(1f));IconButton(onClick={reminderDialog=true}){Icon(Icons.Outlined.NotificationsNone,"Hatırlatma",tint=androidx.compose.ui.graphics.Color.White)};IconButton(onClick={lockDialog=true}){Icon(if(e.note.mode=="none")Icons.Outlined.LockOpen else Icons.Outlined.Lock,"Kilit ayarları")}
        }
        ...
        LazyRow(Modifier.fillMaxWidth().background(SurfaceColor),contentPadding=PaddingValues(horizontal=8.dp),horizontalArrangement=Arrangement.spacedBy(2.dp)) {
            ...
            item { Tool(Icons.Outlined.CheckBox,"Yapılacak ekle"){add("check")} };item { Tool(Icons.Outlined.ContentPaste,"Panodan yapıştır"){val clip=(activity.getSystemService(Context.CLIPBOARD_SERVICE) as android.content.ClipboardManager).primaryClip?.getItemAt(0)?.coerceToText(activity)?.toString().orEmpty();if(clip.isBlank())onMessage("Pano boş") else { val v=editView;if(v!=null&&v.hasFocus()){val st=v.selectionStart.coerceAtLeast(0);val en=v.selectionEnd.coerceAtLeast(st);v.text.replace(st,en,clip)}else{val b=Block(html=TextUtils.htmlEncode(clip).replace("\n","<br>"));e.change(e.content.copy(blocks=e.content.blocks+b));e.active=b.id}}} };item { Tool(Icons.Outlined.Image,"Resim ekle",!busy){e.externalOperation=true;imagePicker.launch("image/*")} };item { Tool(Icons.Outlined.Link,"Bağlantı ekle"){linkDialog=true} }
    ...
    if(searchDialog) { var q by remember { mutableStateOf("") };var hit by remember { mutableIntStateOf(0) };val matches=remember(q,e.version){if(q.isBlank()) emptyList() else e.content.blocks.mapIndexedNotNull { i,b -> if(b.kind!="image"&&Html.fromHtml(b.html,Html.FROM_HTML_MODE_COMPACT).toString().contains(q,true)) i else null }};AlertDialog(onDismissRequest={searchDialog=false},title={Text("Not içinde ara")},text={Column(verticalArrangement=Arrangement.spacedBy(10.dp)){OutlinedTextField(q,{q=it;hit=0},singleLine=true,leadingIcon={Icon(Icons.Outlined.Search,null)},label={Text("Ara")});Text(if(q.isBlank())"Aranacak metni yazın" else "${matches.size} eşleşme",color=Muted);if(matches.isNotEmpty())Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){OutlinedButton(onClick={hit=(hit-1+matches.size)%matches.size;val idx=matches[hit];e.active=e.content.blocks[idx].id;scope.launch{listState.animateScrollToItem(idx+1)}}){Text("Önceki")};Button(onClick={hit=(hit+1)%matches.size;val idx=matches[hit];e.active=e.content.blocks[idx].id;scope.launch{listState.animateScrollToItem(idx+1)}}){Text("Sonraki")}}}},confirmButton={TextButton(onClick={searchDialog=false}){Text("Kapat")}}) }
    ...
    AlertDialog(onDismissRequest=dismiss,title={Text("Not ikonu")},text={
        Column(Modifier.verticalScroll(rememberScrollState())) {
            ...
            iconKeys.chunked(4).forEach { row ->
                Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(8.dp,Alignment.Start)) {
                    row.forEach { key ->
...
@Composable fun ColorPicker(selected:String,change:(String)->Unit) {
    val initial=runCatching { android.graphics.Color.parseColor(selected) }.getOrDefault(android.graphics.Color.parseColor("#C7C7DB"));val hsv=FloatArray(3);android.graphics.Color.colorToHSV(initial,hsv);var hue by remember(selected){ mutableFloatStateOf(hsv[0]) };var sat by remember(selected){ mutableFloatStateOf(hsv[1]) };var value by remember(selected){ mutableFloatStateOf(hsv[2]) }
    fun custom():String { val c=android.graphics.Color.HSVToColor(floatArrayOf(hue,sat,value));return String.format("#%06X",0xFFFFFF and c) }
    Column { palette.chunked(5).forEach { row->Row(Modifier.fillMaxWidth().padding(vertical=6.dp),horizontalArrangement=Arrangement.spacedBy(10.dp,Alignment.Start)) { row.forEach { color->Box(Modifier.size(40.dp).background(hexColor(color),RoundedCornerShape(12.dp)).border(if(selected.equals(color,true))3.dp else 0.dp,MaterialTheme.colorScheme.onSurface,RoundedCornerShape(12.dp)).clickable { change(color) },contentAlignment=Alignment.Center) { if(selected.equals(color,true))Icon(Icons.Outlined.Check,null,tint=contrast(hexColor(color)),modifier=Modifier.size(18.dp)) } } } };Spacer(Modifier.height(8.dp));Text("Özel renk");Box(Modifier.fillMaxWidth().height(42.dp).background(hexColor(custom()),RoundedCornerShape(12.dp)).clickable{change(custom())});Text("Ton",style=MaterialTheme.typography.labelSmall,color=Muted);Slider(hue,{hue=it;change(custom())},valueRange=0f..360f);Text("Doygunluk",style=MaterialTheme.typography.labelSmall,color=Muted);Slider(sat,{sat=it;change(custom())},valueRange=0f..1f);Text("Parlaklık",style=MaterialTheme.typography.labelSmall,color=Muted);Slider(value,{value=it;change(custom())},valueRange=0.05f..1f) }
}
...
@Composable fun ReminderDialog(activity:Activity,existing:String,dismiss:()->Unit,save:(String)->Unit) {
    val initial=remember { runCatching { Reminder.parse(existing) }.getOrNull() };var dateTime by remember { mutableStateOf(initial?.let { LocalDateTime.parse(it.anchor) }?:LocalDateTime.now().plusHours(1).withSecond(0).withNano(0)) };var repeat by remember { mutableStateOf(initial?.repeat?:"once") };var interval by remember { mutableStateOf((initial?.interval?:1).toString()) };var days by remember { mutableStateOf(initial?.weekdays?:listOf(1,2,3,4,5)) };var exact by remember { mutableStateOf(ReminderEngine.allowed(activity)) };var customMessage by remember { mutableStateOf(initial?.message?.isNotBlank()==true) };var message by remember { mutableStateOf(initial?.message.orEmpty()) }
    AlertDialog(onDismissRequest=dismiss,title={Text("Hatırlatma")},text={Column(Modifier.verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(10.dp)) {
        ...
        Text("Bildirim metni",style=MaterialTheme.typography.labelLarge);Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){FilterChip(!customMessage,{customMessage=false},label={Text("Varsayılan")});FilterChip(customMessage,{customMessage=true},label={Text("Özel mesaj")})};if(customMessage)OutlinedTextField(message,{if(it.length<=200)message=it},label={Text("Bildirim mesajı")},supportingText={Text("${message.length}/200")},maxLines=3,modifier=Modifier.fillMaxWidth())
    ...
    }},confirmButton={TextButton(enabled=(interval.toIntOrNull()?:0) in 1..99 && (repeat!="weekdays"||days.isNotEmpty())&&(repeat!="once"||dateTime.isAfter(LocalDateTime.now())),onClick={save(Reminder(dateTime.toString(),repeat,interval.toInt(),days,message=if(customMessage)message.trim() else "").json())}){Text("Kaydet")}},dismissButton={Row { if(existing.isNotBlank())TextButton(onClick={save("")}){Text("Kaldır")};TextButton(onClick=dismiss){Text("Vazgeç")} }})
}
@Composable fun LockDialog(activity:Activity,e:EditorState,dismiss:()->Unit,message:(String)->Unit) {
    var pin by remember { mutableStateOf("") };var confirm by remember { mutableStateOf("") };var useDefault by remember { mutableStateOf(DefaultLock.get()!=null) };var busy by remember { mutableStateOf(false) };var error by remember { mutableStateOf("") };val scope=rememberCoroutineScope()
    DisposableEffect(Unit) { activity.window.addFlags(android.view.WindowManager.LayoutParams.FLAG_SECURE);onDispose { if(e.note.mode=="none")activity.window.clearFlags(android.view.WindowManager.LayoutParams.FLAG_SECURE) } }
    AlertDialog(onDismissRequest={if(!busy)dismiss()},title={Text(if(e.note.mode=="none")"Notu kilitle" else "Kilit ayarları")},text={Column(verticalArrangement=Arrangement.spacedBy(12.dp)) {
        if(e.note.mode=="none") {
            if(DefaultLock.get()!=null){Text("Kilit türü");Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){FilterChip(useDefault,{useDefault=true},label={Text("Varsayılan kilit")});FilterChip(!useDefault,{useDefault=false},label={Text("Yeni kilit")})}}
            if(useDefault&&DefaultLock.get()!=null)Text("Ayarlar'daki varsayılan PIN kullanılacak.",color=Muted) else {Text("6–12 rakamlı yeni bir PIN belirleyin.",color=Muted);OutlinedTextField(pin,{if(it.length<=12&&it.all(Char::isDigit))pin=it},label={Text("PIN")},visualTransformation=PasswordVisualTransformation(),keyboardOptions=androidx.compose.foundation.text.KeyboardOptions(keyboardType=KeyboardType.NumberPassword),singleLine=true);OutlinedTextField(confirm,{if(it.length<=12&&it.all(Char::isDigit))confirm=it},label={Text("PIN tekrar")},visualTransformation=PasswordVisualTransformation(),keyboardOptions=androidx.compose.foundation.text.KeyboardOptions(keyboardType=KeyboardType.NumberPassword),singleLine=true)}
        } else { Text("İçerik şifreli. Bildirimler ve widget içeriği göstermez.",color=Muted);OutlinedButton(enabled=!busy,onClick={e.meta(e.note.copy(mode="none",salt="",wrapped="",bio="",payload=e.content.json()));dismiss();message("Notun kilidi kaldırıldı")}){Text("Kilidi kaldır")} }
    ...
    }},confirmButton={if(e.note.mode=="none")TextButton(enabled=!busy&&((useDefault&&DefaultLock.get()!=null)||(pin.length>=6&&pin==confirm)),onClick={busy=true;scope.launch { val chosen=if(useDefault)DefaultLock.get().orEmpty() else pin;val mode=if(useDefault)"default" else "pin";val result=withContext(Dispatchers.IO){runCatching { Vault.lock(e.note,e.content,chosen,mode) }};result.onSuccess { (n,key)->e.installKey(key);e.meta(n);e.persist();if(e.saved){busy=false;dismiss();message(if(mode=="default")"Varsayılan kilit etkin" else "PIN kilidi etkin")}else{error=e.savingError;busy=false} }.onFailure { error=it.localizedMessage?:"Kilit oluşturulamadı";busy=false } }}){Text("Kilitle")}else TextButton(enabled=!busy,onClick=dismiss){Text("Tamam")}},dismissButton={if(e.note.mode=="none")TextButton(enabled=!busy,onClick=dismiss){Text("Vazgeç")}})
}
